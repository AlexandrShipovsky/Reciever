...
int c_decode0(void)
{
	...
	if (degs2 < 0)
		return 0; // If the polynom of errors of the form z^degs, then error
	c_clear(LAM1, t2 + 1);
	c_clear(LAM2, t2 + 1);
	c_clear(OMEGA1, t2 + 1);
	c_clear(OMEGA2, t2 + 1);
	...
	while ((dom2 >= t) && (dlam2 <= t))
		c_Evklid1();
	// The polynoms LAM2 � OMEGA2 are ready.
...
{
	int i, j, t0 = NErr;
	if (bufsize > 255 || bufsize < 3)
		return -1;
	bufs = bufsize;
	if (t0 < 0 || 2 * (t0 + 1) >= bufsize)
		return -1;
	if (t0 > TMax)
		return -1;
	if (t == t0)
	...
	c_g[0] = 1;
	c_g[1] = 1;
	c_g[2] = 0; // initial polynomial (x-1)
